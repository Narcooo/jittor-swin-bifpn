...
class Runner:
    def __init__(self):
        ...
        # cfg.pretrained_weights = '/work_dirs/faster_rcnn_RoITrans_swint_fpn_1x_dota_1_' \
        #               'plus_with_flip_rotate_balance/checkpoints/ckpt_11.pkl',
        if (cfg.pretrained_weights):
            self.load_init(cfg.pretrained_weights, model_only=True)
    ...
    def load(self, load_path, model_only=False):
        ...
        if (not model_only):
            ...
            # self.scheduler.base_lr = self.scheduler.base_lr / 100

        if ("model" in resume_data):
            self.model.load_parameters(resume_data["model"])
            # self.model.load('/work_dirs/test.pkl')
    ...
    def load_init(self, load_path, model_only=False,
                  detector_need = True,
                  neck_need = True):
        ...
        resume_data = jt.load(load_path)
        # if 'model' in resume_data:
        tmp_model_dict = resume_data['model'].copy()
        #     pickle.dump(resume_data['model'],open('/work_dirs/test.pkl', "wb"))
        if not detector_need:
            if 'model' in resume_data:
                for i in resume_data['model']:
                    if i.split('.')[0] == 'backbone' or i.split('.')[0] == 'neck':
                        continue
                    else:
                        del tmp_model_dict[i]
        if not neck_need:
            if 'model' in resume_data:
                for i in resume_data['model']:
                    if i.split('.')[0] == 'neck':
                        del tmp_model_dict[i]
        resume_data['model'] = tmp_model_dict.copy()
        if (not model_only):
            meta = resume_data.get("meta",dict())
            self.epoch = meta.get("epoch",self.epoch)
            self.iter = meta.get("iter",self.iter)
