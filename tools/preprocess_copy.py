...
from jdet.data.devkits.fair1M_label_to_fair1M_5 import fair1m_label_to_fair_1m_5
...
def run(cfg):
    ...
    if (cfg.type=='FAIR' or cfg.type=='FAIR1M_1_5'):
        for task in cfg.convert_tasks:
            ...
            if cfg.Fair2Fair1M_5 and task != "test":
                print('processing Fair1M labels to Fair1M5 labels')
                fair1m_label_to_fair_1m_5(os.path.join(cfg.source_dataset_path, task))

    if cfg.Fair2Fair1M_5:
        cfg.type='FAIR1M_1_5'
...
def main():
    ...
    parser.add_argument(
        "--config-file",
        default="configs/preprocess/fair_preprocess_config_1024.py",
        metavar="FILE",
