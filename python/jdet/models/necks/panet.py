...
class PANet(nn.Module):
    ...
    def execute(self, inputs):
        ...
        if self.panet_buttomup:
            fpn_middle_blobs = []
            for oo in range(len(outs)):
                if oo==0:
                    fpn_middle_blobs.append(outs[oo])
                else:
                    tmp_fm=self.panet_buttomup_conv1_modules[oo-1](fpn_middle_blobs[oo-1])
                    # fm_new = jt.concat((tmp_fm,outs[oo]), dim=1)
                    fm_new = tmp_fm+outs[oo]
                    fpn_middle_blobs.append(self.panet_buttomup_conv2_modules[oo-1](fm_new))
            outs=fpn_middle_blobs
