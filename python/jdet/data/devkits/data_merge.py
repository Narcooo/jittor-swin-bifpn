...
def data_merge_result(result_pkl,work_dir,epoch,name,dataset_type,images_dir=""):
    ...
    if (dataset_type == 'FAIR'):
        ...
        if is_win():
            ...
            os.system(f"cp {os.path.join(final_path, name+'.csv')} {csv_path}")
    else:
        if is_win():
            files = glob.glob(os.path.join(final_path,"*"))
            with zipfile.ZipFile(zip_path, 'w',zipfile.ZIP_DEFLATED) as t:
...
if __name__ == "__main__":
    work_dir = "/work_dirs/faster_rcnn_RoITrans_swint_fpn_1x_fair_15_plus_with_flip_rotate_balance"
    epoch = 12
    ...
    data_merge(result_pkl, save_path, final_path, "FAIR1M_1_5")